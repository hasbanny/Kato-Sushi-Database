 ...
 *
 *
 * @method     ChildSupplierQuery orderBySupId($order = Criteria::ASC) Order by the sup_id column
 ...
 * @method     ChildSupplierQuery orderByPhoneNum($order = Criteria::ASC) Order by the phone_num column
 *
 * @method     ChildSupplierQuery groupBySupId() Group by the sup_id column
 ...
 * @method     ChildSupplierQuery groupByPhoneNum() Group by the phone_num column
 *
...
abstract class SupplierQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT sup_id, name, address, phone_num FROM supplier WHERE sup_id = :p0';
        try {
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        return $this->addUsingAlias(SupplierTableMap::COL_SUP_ID, $key, Criteria::EQUAL);
    }
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        return $this->addUsingAlias(SupplierTableMap::COL_SUP_ID, $keys, Criteria::IN);
    }

    /**
     * Filter the query on the sup_id column
     *
     * Example usage:
     * <code>
     * $query->filterBySupId(1234); // WHERE sup_id = 1234
     * $query->filterBySupId(array(12, 34)); // WHERE sup_id IN (12, 34)
     * $query->filterBySupId(array('min' => 12)); // WHERE sup_id > 12
     * </code>
     *
     * @param     mixed $supId The value to use as filter.
     ...
     * @return $this|ChildSupplierQuery The current query, for fluid interface
     */
    public function filterBySupId($supId = null, $comparison = null)
    {
        if (is_array($supId)) {
            $useMinMax = false;
            if (isset($supId['min'])) {
                $this->addUsingAlias(SupplierTableMap::COL_SUP_ID, $supId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($supId['max'])) {
                $this->addUsingAlias(SupplierTableMap::COL_SUP_ID, $supId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(SupplierTableMap::COL_SUP_ID, $supId, $comparison);
    }
    ...
    {
        if ($supplier) {
            $this->addUsingAlias(SupplierTableMap::COL_SUP_ID, $supplier->getSupId(), Criteria::NOT_EQUAL);
        }
